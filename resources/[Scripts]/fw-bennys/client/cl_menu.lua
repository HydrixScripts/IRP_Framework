...
function BuildMenu(Vehicle)
    ...
    if BodyHealth < 1000.0 then
        ...
        if CurrentBennyZone and CurrentBennyZone.data.IsWheelfitment then
        ...
        for i = 1, #Config.Menus, 1 do
            ...
            -- local VehicleRecord = FW.SendCallback("fw-police:server:getVehicleRecord", GetVehicleNumberPlateText(Vehicle))
            -- local IsVinScratched = VehicleRecord ~= nil and VehicleRecord[0] ~= nil and VehicleRecord[0].vinscratched == 1
            if exports['fw-vehicles']:IsGovVehicle(Vehicle) and Data.Disabled.Emergency then goto Skip end
...
function BuildFitmentMenu(Vehicle)
    ...
    Citizen.CreateThread(function()
        ...
        exports['fw-vehicles']:SetVehicleMeta(Vehicle, "WheelFitment", {
...
RegisterNUICallback("OpenTargetMenu", function(Data, Cb)
    ...
    if Menu.GetMenu(Data.TargetMenu) ~= nil then
        ...
        print(("Bennys/OpenTargetMenu: Can't open target menu '%s'. (Current: %s)"):format(Data.TargetMenu, Data.CurrentMenu))
    end
    Cb('Ok')
end)
