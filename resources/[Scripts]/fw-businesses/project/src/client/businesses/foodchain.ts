...
on("fw-businesses:Client:Foodchain:ShowEmployees", async (Data: {
    ...
    for (let i = 0; i < Result.length; i++) {
        ...
        ContextItems.push({
            Icon: 'user-headset',
            Title: `(#${Cid}) ${Name}`,
            Desc: `${MinutesClockedIn} ${MinutesClockedIn == 1 ? "minute" : "minutes"} in service.`
        });
    };
...
onNet("fw-items:Client:Used:MysteryBox", async (Item: any) => {
    ...
    const Finished = await FW.Functions.CompactProgressbar(3000, "Opening Mystery Box..", false, true, {disableMovement: false, disableCarMovement: false, disableMouse: false, disableCombat: false}, {}, {}, {}, false)
